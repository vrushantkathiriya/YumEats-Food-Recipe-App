...
const Featured = () => {
    ...
    const [currentIndex, setCurrentIndex] = useState(0)
          ...
          const prevSilder = () => {
            ...
            const newIndex = isFirstSlice ? sliders.length - 1 : currentIndex - 1
  ...
  return (
    <div className='max-w-[1920px] h-[700px] w-full py-4 px-4 relative group'>
        <div className=' h-full w-full rounded-2xl bg-center bg-cover duration-500'
        ...
        <div className='hidden group-hover:block absolute top-[50%] -translate-x-0 translate-y-[-50%] left-5  text-2xl rounded-full p-2 bg-orange-700 text-white cursor-pointer'>
            <BsChevronCompactLeft onClick={prevSilder} />
        </div>
        ...
        <div className='flex top-4 justify-center py-2'>
          {
              sliders.map((sliderItems, sliderIndex) => (
                <div 
                  key={sliderIndex}
                  onClick={()=>moveToNextSlider (sliderIndex)}
